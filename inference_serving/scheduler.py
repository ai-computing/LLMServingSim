...
class Scheduler:
    ...
    def schedule_base(self, current, sys, batch_id=-1):
        ...
        if sys == self.start_npu:
            ...
            if self.enable_chunked_prefill:
                decode_tokens = sum(1 for req in batch_req if not req.is_init)
                chunk_budget = max(0, self.max_num_batched_tokens - decode_tokens)
            for req in batch_req:
                if req.is_init:
                    if self.enable_chunked_prefill:
                        remaining = req.input - req.processed_tokens
                        chunk_q = min(remaining, max(1, chunk_budget))
                        chunk_budget = max(0, chunk_budget - chunk_q)
                        total_len += chunk_q
                        if req.processed_tokens == 0:
                            req.set_que_delay(current)
                        q_list.append(chunk_q)
                        prefill_q_list.append(chunk_q)
                        prefill_k_list.append(req.processed_tokens)
                    else:
                        total_len += req.input
                        req.set_que_delay(current)
    ...
    def schedule_with_prefix(self, current, sys, batch_id=-1):
        if sys == self.start_npu:
            ...
            for req in batch_req:
                ...
                if req.is_init:
                    effective_input = max(req.input - req.prefix_cache_hit, 1)
                    if self.enable_chunked_prefill:
                        remaining = effective_input - req.processed_tokens
                        chunk_q = min(remaining, max(1, chunk_budget))
                        chunk_budget = max(0, chunk_budget - chunk_q)
                        total_len += chunk_q
                        if req.processed_tokens == 0:
                        ...
                        if self.enable_prefix_caching and req.prefix_cache_hit > 0:
                            hit_len += req.prefix_cache_hit
                        q_list.append(effective_input)
                        num_prefill += 1
                        prefill_q_list.append(effective_input)
                        prefill_k_list.append(0)
                else:
                    total_len += 1
                    q_list.append(1)
    ...
    def add_done(self, id, sys, finish):
        ...
        for req in batch.requests:
            ...
            if req.is_init:
                if self.enable_chunked_prefill:
                    chunk_q = batch.prefill_q_list[prefill_chunk_idx]
                    prefill_chunk_idx += 1
                    req.processed_tokens += chunk_q
                    if req.processed_tokens < req.input:
                        # Prefill not yet complete; requeue for next chunk
                        pool.append(req)
                        continue
                req.is_init = False
                if self.pd_type != "prefill":
